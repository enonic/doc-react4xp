...
See the <<#invocation-example, invocation example>> above for the minimal case, and the <<#react4xp_object, React4xp object>> when you need the two rendering steps separately.
...
Typically used inside a data processor, like in the https://developer.enonic.com/learn/react4xp-tutorial[React4XP tutorial].
...
If a processor throws, React4XP does not fail the request. The error is logged server-side and the offending component is replaced with an error component (`type: 'error'`) carrying an HTML message naming the descriptor, content path and the sanitized error message, so the rest of the page still renders.

What visitors see depends on the request mode. In Content Studio (edit, inline and admin mode) `BaseComponent` renders the error component as a red box, so editors can see what failed and where. In live and preview mode nothing is rendered in its place, and the page is shown without the failing component.

The error view is not customizable through the `ComponentRegistry`. To render your own message, either check for `component.type === 'error'` in the <<app#, app entry>> before handing off to `BaseComponent` (this covers the top-level component only, since `Region` renders nested components directly), or catch the error inside the processor and let the part's `View` handle it. Treat a rendered error component as a processor that needs fixing, not as a supported rendering path.

Since the request still completes with status 200, the application's XP error handler is not invoked for processor errors. XP invokes it only when the app function throws, or returns a 4xx or 5xx response without a body.

NOTE: If the React entry itself fails during server-side rendering, React4XP inserts a "React4xp SSR error" box in place of the entry instead. In live mode the box shows only the entry name and ID, and the error message is logged server-side. See <<ssr#, SSR and hydration>>.
...
The methods and options are documented in full below, starting with the <<#constructor, constructor>>.
...
-> See <<#react4xp_object_request, Request and automated behavior>> for the custom flow examples.
====
...
TIP: For a complete custom flow function, see the <<#react4xp_object, usage example>> at the start of this section.
