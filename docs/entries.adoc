...
When rendered (whether it's by using `.render` or a pair of `renderBody`/`renderPageContributions` in the https://developer.enonic.com/learn/react4xp-tutorial[custom flow syntax]), each entry will become not only assets of their own, but *independent root level react apps* in the browser memory! Good to know if you display more than one entry on a page - which you can, but you might also consider using just one entry for the root, and multiple imports in it.
...
. If you use `portal.getComponent()` to insert a component object (like `entry` in https://developer.enonic.com/learn/react4xp-tutorial[the first example]), you can ignore the jsxPath and let the component object do the job: `React4xp.render` will use it to look for an entry with *the same name, in the same folder*, as a part/page/layout implementation.
