...
This also means that the <Region> component (part of @enonic/react-components) has been changed, and is now a pure React component, handling the rendering of its sub-components directly, rather than relying on the Site engine's placeholder and iterative rendering technique.

The latter is a breaking change. However, React4XP is still able to render multiple entries on a page (i.e. only render your parts with React4XP) if needed. Check out the <<legacy-upgrade#, legacy mode upgrade>>.
...
To migrate a legacy React4XP application we have written detailed <<upgrade#, upgrade notes>>.
