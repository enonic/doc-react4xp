...
A minimal controller passes an <<entries#, entry>>, `props`, and the `request` to `render`. The return value is an XP response object you return directly:
...
| `entry` | string \| component object | Reference to an <<entries#, entry>>: the react component to be rendered. <<entries#jsxPath, JsxPath>> reference - or if the entry is a TSX/JSX file with the _same name_ in the _same folder_ as the controller, you can use a `portal.getComponent()` object here. Corresponds to the template argument in `thymeleaf.render`.
...
NOTE: Instantiate the `DataFetcher` once in its own module and import that instance where needed - both in the <<controller#, app controller>> and in the React entry via `componentRegistry`. The constructor takes no arguments.
...
Pass the whole result straight to `render` - see the <<controller#, app controller>>:
...
Returns an HTML string ready to return as the `body` attribute in a https://developer.enonic.com/docs/code/stable/web/request-response[response object] from the controller.
...
A regular https://developer.enonic.com/docs/code/stable/web/sites/contributions[page contributions] object, ready to be used as the `pageContributions` attribute in an https://developer.enonic.com/docs/code/stable/web/request-response[response object] from the controller.
