...
In. React4XP we call these `entries`. It's actually a webpack term, but in React4xp we use the word entry for *a React component that can be called by React4xp* from an XP script file. They are the only react files that React4xp can use directly.
...
(Here's a <<entries/cheatsheet#, cheat sheet>>. See also chunks, for a <<chunks#entries_and_chunks, complete picture>>)

On the XP/server side, you have the server-side functions with their backend XP flow, logic and XP lib imports. Entries are the beginning of the frontend side (although the same code is also used for serverside-rendering): they do regular frontend-style ES6 logic and can *import* stuff from NPM/node_modules, your own logic, nested react components, whatever - even other entries.
...
Entries are pretty much just standard TSX/JSX files, but they must follow *two requirements* (another <<entries/howto#, cheat sheet>>):

1. is located in a folder below one of the *entryDirs* folders listed in _react4xp.config.js_ - the starter uses *_react4xp/entries/_* (see also <<entries#jsxPath, jsxPath>>).
In legacy XP 7 apps, entries could also be placed below *_/site/_* - XP 8 apps no longer have this folder (it was renamed to `cms/`, which is not scanned for entries),
...
For example, the starter's entry _react4xp/entries/App.tsx_ is compiled into the entry asset _App.js_. Add a second entry _react4xp/entries/shop/Cart.tsx_, and it becomes _shop/Cart.js_: each entry gets its own asset, named from its path below the <<build#entryDirs, entryDirs>> folder - see <<#jsxPath, jsxPath>>.

When rendered (whether it's by using `.render` or a pair of `renderBody`/`renderPageContributions` in the <<library#react4xp_object, custom flow>>), each entry will become not only assets of their own, but *independent root level react apps* in the browser memory! Good to know if you display more than one entry on a page - which you can, but you might also consider using just one entry for the root, and multiple imports in it.



[[classes-and-hooks]]
== Classes and hooks in entries
If your entry is a https://reactjs.org/docs/react-component.html[react class component] or a functional component that uses https://reactjs.org/docs/hooks-intro.html[react hooks] (and possibly other specific react features?), the default-export must be correctly wrapped.
...
In the examples above it would be equivalent to `Welcome(props);` and `HookButton(props);`. `Welcome` is not compiled to a function that can be called like this, and the pure `HookButton` function is not really a _functional component_ (before it's called as a component, the way we do when it's wrapped), just a regular JS function - so the straight export https://reactjs.org/warnings/invalid-hook-call-warning.html[breaks the rules of hooks and you get an error].
...
. If you use `portal.getComponent()` to insert a component object (like `entry` in the https://developer.enonic.com/learn/react4xp-tutorial[React4XP tutorial]), you can ignore the jsxPath and let the component object do the job: `React4xp.render` will use it to look for an entry with *the same name, in the same folder*, as a part/page/layout implementation.
...
[start=2]
. Using the setup from the starter, put your entries under _src/main/resources/_ *_react4xp/entries_*.
Then the jsxPath (and compiled-asset path) will be relative to _/entries/_.
* Actually, you can control this with the `entryDirs` property in your local https://github.com/enonic/enonic-react4xp/blob/master/examples/react4xp.config.js[_react4xp.config.js_]. The starter has added `entryDirs:['entries']` here, but this is a comma-separated list of folder names relative to _src/main/resources/react4xp/_. Remember, if you add more entryDirs or replace `entries`, the jsxPaths will always be relative to those new entryDirs, so beware of name collisions.
...
. In legacy XP 7 apps only: if an entry's source file is a TSX/JSX under _/src/main/resources/_ *_site/_*, the jsxPath is the source file's path relative to _/src/main/resources/_ - unix-style and without file extension.
...
XP 8 apps have no _site/_ folder, so this rule no longer applies to them.
