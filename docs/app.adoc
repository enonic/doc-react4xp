...
Declare one export per HTTP method the function handles - `GET`, `POST`, `PUT`, `DELETE`, `PATCH`, `HEAD`, `OPTIONS` - or `all` as a fallback for any method without its own export. See https://developer.enonic.com/docs/code/stable/web/request-response[Request & Response] for the full contract.
