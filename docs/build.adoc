...
You may test how your application will perform on a server by launching your sandbox in production mode. In this mode, assets will be served with optimized cached headers:
...
React4xp will look for (ie. sets up webpack to look for) .TSX/.JSX files to turn into <<entries#, entries>> below the folders listed in `entryDirs` (relative to _src/main/resources/react4xp/_).
The starter sets `entryDirs: ['entries']`, so entries are picked up from _src/main/resources/react4xp/entries/_.

Adding comma separated values under `entryDirs` adds _more_ folder names/paths where TSX/JSX files will also become entries.

NOTE: In legacy XP 7 apps, React4xp also scanned _src/main/resources/site/_ for entries by default.
XP 8 apps no longer have this folder (it was renamed to `cms/`, which is not scanned for entries).


[[chunkDirs]]
=== chunkDirs

By default, React4xp will look for (ie. sets up webpack to look for) resources _imported by_ the entries. These are https://webpack.js.org/concepts/[bundled] into separate assets that React4xp automatically loads at both server- and clientside rendering: <<chunks#, chunks>>, by this pattern:
...
- Most commonly, the built-in webpack setup is pretty minimal, only adding https://webpack.js.org/loaders/[loaders] for compiling react from TSX/JSX. It's likely you will need to add loaders of your own, maybe use additional plugins etc
