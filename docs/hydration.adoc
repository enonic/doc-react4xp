= Hydration - Advanced client side rendering
:toc: right
:imagesdir: media/

By default, React4XP renders entries both <<ssr#, server-side>> and client-side: the HTML is rendered on the server first, then hydrated (activated) in the browser. You can also switch to server-side only, or client-side only.
...
Visit the <<library#, library reference>> for more details.
